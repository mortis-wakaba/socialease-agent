...
def _enable_summary_consent(user_id: str) -> None:
...
async def test_helpful_strategy_auto_commit_is_idempotent() -> None:
    ...
    _enable_summary_consent(user_id)
    ...
    memories = repository_factory().long_term_memory_repository().list_memories(
        user_id
    )
...
async def test_explicit_revoke_uses_exact_user_scoped_hash_and_is_idempotent() -> None:
    ...
    events = repository_factory().long_term_memory_repository().list_events(
        user_id=user_id
    )
...
async def test_revoke_without_exact_target_is_rejected_without_body_storage() -> None:
    ...
    exported = str(MemoryPrivacyService().export(user_id).model_dump())
    events = repository_factory().long_term_memory_repository().list_events(
        user_id=user_id
    )
...
async def test_social_context_is_saved_as_pending_not_committed() -> None:
    ...
    pending = repository_factory().memory_proposal_repository().list_pending(user_id)
...
async def test_crisis_and_no_consent_skip_extraction_entirely() -> None:
    ...
    _enable_summary_consent(crisis_user)
