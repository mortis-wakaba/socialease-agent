...
class ConflictOnceRepository:
    ...
    async def save_checkpoint(self, checkpoint, **kwargs):
        if self.conflicts == 0:
            self.conflicts += 1
            raise MemoryConflictError("simulated concurrent writer")
        return await self.delegate.save_checkpoint(checkpoint, **kwargs)
...
@pytest.mark.anyio
async def test_checkpoint_lifecycle_is_versioned_minimized_and_audited() -> None:
    user_id = f"checkpoint_lifecycle_{uuid4().hex}"
    thread_id = f"thread_{uuid4().hex}"
    repository = repository_factory().long_term_memory_repository()
    service = _service(repository=repository)

    started = await service.record_roleplay(
        user_id=user_id,
        thread_id=thread_id,
    ...
    progressed = await service.record_roleplay(
        user_id=user_id,
        thread_id=thread_id,
        scenario=_scenario("dorm_conflict"),
...
@pytest.mark.anyio
async def test_restore_requires_consent_exact_owner_thread_scenario_status_and_ttl() -> None:
    ...
    assert (
        await service.restore_roleplay_context(
            user_id=owner,
            thread_id=thread_id,
...
@pytest.mark.anyio
async def test_active_checkpoint_has_independent_hard_token_budget() -> None:
...
@pytest.mark.anyio
async def test_checkpoint_write_retries_one_conflict_and_blocks_terminal_reactivation() -> None:
    ...
    completed = await service.record_roleplay(
        user_id=user_id,
        thread_id=thread_id,
    ...
    invalid = await service.record_roleplay(
        user_id=user_id,
        thread_id=thread_id,
        scenario=_scenario("classroom_speech"),
    ...
    assert (await delegate.get_checkpoint(thread_id, user_id)).status == (
