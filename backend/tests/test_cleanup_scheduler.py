...
@pytest.mark.anyio
async def test_cleanup_scheduler_run_once_calls_retention_service() -> None:
...
@pytest.mark.anyio
async def test_cleanup_scheduler_dry_run_does_not_call_service() -> None:
    service = FakeRetentionService()
    scheduler = CleanupScheduler(
        service,  # type: ignore[arg-type]
...
@pytest.mark.anyio
async def test_cleanup_scheduler_run_forever_respects_max_runs_and_interval() -> None:
    service = FakeRetentionService()
    sleeps: list[float] = []
    ...
    runs = await scheduler.run_forever(max_runs=3)
...
@pytest.mark.anyio
async def test_cleanup_scheduler_continues_after_iteration_error() -> None:
    ...
    runs = await scheduler.run_forever(max_runs=2)
...
@pytest.mark.anyio
async def test_cleanup_scheduler_skips_when_another_replica_holds_lock() -> None:
    service = FakeRetentionService()
...
@pytest.mark.anyio
async def test_cleanup_scheduler_releases_lock_after_failure() -> None:
    service = FakeRetentionService(fail=True)
