...
def test_postgres_protocol_create_get_and_transition(
    repository: PostgresProtocolRepository,
) -> None:
    user_id = f"pg_protocol_user_{uuid4().hex}"
    protocol = repository.create(
        user_id=user_id,
    ...
    fetched = repository.get_for_user(protocol.protocol_id, user_id)
    ...
    stale_transition = repository.transition_status(
        protocol_id=protocol.protocol_id,
        user_id=user_id,
        expected_status=ProtocolStatus.PENDING,
...
def test_postgres_protocol_consume_is_atomic(
    repository: PostgresProtocolRepository,
) -> None:
    ...
    def consume_once() -> bool:
        consumed = repository.transition_status(
            protocol_id=protocol.protocol_id,
            user_id=user_id,
...
def test_postgres_protocol_expire_pending_before(
    repository: PostgresProtocolRepository,
) -> None:
    ...
    expired_count = repository.expire_pending_before(datetime.now(timezone.utc))
    fetched = repository.get_for_user(protocol.protocol_id, user_id)
