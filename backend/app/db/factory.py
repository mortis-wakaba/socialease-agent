...
from app.db.postgres.memory_privacy_repository import (
    PostgresMemoryPrivacyRepository,
)
...
from app.memory.privacy_repository import MemoryPrivacyRepository
...
class RepositoryFactory:
    ...
    def memory_privacy_repository(self) -> MemoryPrivacyRepository:
        """Return the PostgreSQL user-data export and erasure adapter."""
        if self.provider == DatabaseProvider.POSTGRES:
            return PostgresMemoryPrivacyRepository(
                database_url=self.database_url
            )
        raise self._not_implemented("memory privacy")
    ...
    def intervention_plan_repository(self) -> InterventionPlanRepository:
